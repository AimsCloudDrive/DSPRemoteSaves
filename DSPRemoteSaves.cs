...
namespace DSPRemoteSaves
{
    ...
    public class DSPRemoteSaves : BaseUnityPlugin
    {
        ...
        public async static void OnGameStart()
        {
            if (!CheckConfig() || !S) return;
        ...
        public static async void OnGameExit()
        {
            if (!CheckConfig() || !S) return;
            await UploadSaves();
        }
        ...
        private static async Task<List<RemoteFile>> GetRemoteFileList()
        {
            try
            {
                ...
                var result = JsonConvert.DeserializeObject<FileListResponse>(responseString, JsonSettings);
        ...
        private static async Task<(string FileId, int ChunkSize)> InitUpload(string fileName, long fileSize)
        {
            try
            {
                ...
                var responseString = await response.Content.ReadAsStringAsync();
                var result = JsonConvert.DeserializeObject<UploadInitResponse>(responseString, JsonSettings);
        ...
        private static async Task<bool> CompleteUpload(string fileId)
        {
            try
            {
                ...
                var responseString = await response.Content.ReadAsStringAsync();
                var result = JsonConvert.DeserializeObject<UploadCompleteResponse>(responseString, JsonSettings);
        ...
        private static async Task HandleErrorResponse(HttpResponseMessage response)
        {
            try
            {
                ...
                try
                {
                    var errorResponse = JsonConvert.DeserializeObject<BaseResponse>(responseString, JsonSettings);
        ...
        private static async Task<HttpResponseMessage> PostJson<T>(string endpoint, T payload)
        {
            try
            {
                ...
                staticLogger.LogDebug($"Sending to {endpoint}: {json}");
